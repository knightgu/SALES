...
#' 
#' Note that the objective function in \code{cpernet} is
#' \deqn{w*\Psi(y-X\beta,0.5)/N + \Psi(y-X\beta-X\theta,\tau)/N +
#' \lambda_1*|\beta| + 0.5*\lambda_2*||\beta||^2 + \mu_1*|\theta| +
#' 0.5*\mu_2*||\theta||^2,}{w*\Psi(y-X\beta,0.5)/N +
#' \Psi(y-X\beta-X\theta,\tau)/N + \lambda1*|\beta| + 0.5*\lambda2*||\beta||^2
#' + \mu1*|\theta| + 0.5*\mu2*||\theta||^2,} where
...
#' \code{max(lambda)} with \code{max(lambda)} being the smallest value of
#' \code{lambda} for which all coefficients are zero. The default value depends
#' on the relationship between \eqn{N} (the number of observations) and \eqn{p}
#' (the number of predictors). If \eqn{N < p}, the default is \code{0.01}. If
#' \eqn{N > p}, the default is \code{0.0001}, closer to zero.  A very small
#' value of \code{lambda.factor} will lead to a saturated fit. The argument
#' takes no effect if there is a user-supplied \code{lambda} sequence.
...
#' @param pf.mean,pf.scale L1 penalty factor of length \eqn{p} used for
#' adaptive LASSO or adaptive elastic net. Separate L1 penalty weights can be
#' applied to each mean or scale coefficient to allow different L1 shrinkage.
#' Can be 0 for some variables, which imposes no shrinkage and results in that
#' variable being always included in the model. Default is 1 for all variables
#' (and implicitly infinity for variables listed in \code{exclude}).
...
#' mean or scale coefficient to allow different L2 shrinkage. Can be 0 for some
#' variables, which imposes no shrinkage. Default is 1 for all variables.
...
#' example once \eqn{\beta} enters the model, no matter how many times it exits
#' or re-enters the model through the path, it will be counted only once.
...
#' that produced this object.} \item{b0, t0}{intercept sequences both of length
#' \code{length(lambda)} for the mean and scale respectively.} \item{beta,
#' theta}{\code{p*length(lambda)} matrices of coefficients for the mean and
#' scale respectively, stored as sparse matrices (\code{dgCMatrix} class, the
#' standard class for sparse numeric matrices in the \code{Matrix} package). To
#' convert them into normal R matrices, use \code{as.matrix()}.}
...
#' coefficient matrices.} \item{npasses}{total number of iterations summed over
