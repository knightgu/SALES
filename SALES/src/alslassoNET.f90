...
! ----------------------------- alslassoNETpath ----------------------------- !
SUBROUTINE alslassoNETpath(tau, lam2, maj, nobs, nvars, x, y, ju, pf, pf2,&
     & dfmax, pmax, nlam, flmin, ulam, eps, maxit, nalam, b0, beta, m, nbeta,&
     & alam, npass, jerr, intr)

  IMPLICIT NONE
  ! -------- INPUT VARIABLES -------- !
  INTEGER :: nobs, nvars, dfmax, pmax, nlam, maxit, nalam, npass, jerr, intr
  ...
  DOUBLE PRECISION :: tmp, bigm, d, dif, oldb, al, flmin, dl(nobs)
  DOUBLE PRECISION :: alf = 1.0D0, u = 0.0D0, v
  DOUBLE PRECISION,  DIMENSION(:),  ALLOCATABLE :: b, oldbeta, r
  INTEGER :: i, k, j, l, vrg, ctr, ierr, ni, me, mnl
  ...
  DO l = 1, nlam
     ...
     DO
        ...
        DO
           ...
           IF (intr == 1) THEN
              ...
              DO ! BEGIN GRADIENT DESCENT (NEWTON-RAPHSON)
                 DO i = 1, nobs
                    ...
                    END IF
                 END DO
                 d = SUM(dl) / (nobs * tau + (1.0D0 - 2.0D0 * tau) * COUNT(r <&
                      & 0.0D0))
                 IF (bigm * d**2 < eps) EXIT
                 b(0) = b(0) + d
                 r = r - d
           ...
           !             END IF
           !          END DO
           !          d = SUM(dl) / (nobs * tau + (1.0D0 - 2.0D0 * tau) *&
           !               & COUNT(r < 0.0D0))
           !          IF (bigm * d**2 < eps) EXIT
           !          b(0) = b(0) + d
           !          r = r - d
